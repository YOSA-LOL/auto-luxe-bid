...
import { getGoogleAuthUrl, getUser, mockSignIn } from "@/lib/auth";
import { Button } from "@/components/ui/button";
import { Gavel, FlaskConical } from "lucide-react";
...
const HAS_GOOGLE = !!import.meta.env.VITE_HAS_GOOGLE;
...
function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast.error("Google sign-in not configured yet. Use the demo login below.");
        ...
        <div className="glass-strong rounded-3xl border border-border/60 p-10 shadow-elegant text-center">
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              disabled={loadingGoogle || loadingMock}
            ...
            <Button
              ...
              disabled={loadingGoogle || loadingMock}
              variant="outline"
              className="w-full h-12 glass gap-3 rounded-xl border-primary/30 hover:border-primary/60"
            >
              {loadingMock ? (
                <span className="h-5 w-5 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />
              ) : (
                <FlaskConical className="h-5 w-5 text-primary-glow" />
              )}
              {loadingMock ? "Signing in…" : "Demo login (no Google needed)"}
            </Button>
          </div>
          ...
          <p className="mt-6 text-xs text-muted-foreground leading-relaxed">
            ...
            <button
              onClick={() => toast.info("Terms of Service: apexauto.com/terms")}
            ...
            {" "}and{" "}
            <button
              onClick={() => toast.info("Privacy Policy: apexauto.com/privacy")}
              className="underline underline-offset-2 hover:text-foreground transition-smooth"
            >
              Privacy Policy
            </button>.
