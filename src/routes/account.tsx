...
import { getBidsForUserWithStatus } from "@/lib/cars.server";
...
import {
  ...
  Trophy, CircleX, Bell, Search, Eye, Bookmark, X, CheckCircle2,
} from "lucide-react";
...
export const Route = createFileRoute("/account")({
  head: () => ({ meta: [{ title: "My Account — APEXAuto" }] }),
  loader: async () => {
    const [bids, allCars] = await Promise.all([
      getBidsForUserWithStatus({ data: "You" }),
      getCarsFromDb(),
    ]);
    return { bids, allCars: allCars.map(dbCarToApp) };
  },
  component: AccountPage,
});

function AccountPage() {
  const { bids, allCars } = Route.useLoaderData();
  ...
  const [activeTab, setActiveTab] = useState<"bids" | "profile" | "notifications" | "recent" | "searches">("bids");
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="rounded-3xl glass-strong border border-border/60 p-6 md:p-8 mb-6 shadow-elegant">
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Badge variant="outline" className="mt-2 glass border-primary/30 gap-1 text-xs">
                <CircleCheck className="h-3 w-3 text-primary-glow" /> Verified Buyer
              </Badge>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-6 pt-6 border-t border-border/40">
            {[
              { icon: Gavel, label: "Total Bids", value: bids.length },
              { icon: Car, label: "Cars Bid On", value: uniqueCars },
              { icon: Radio, label: "Active", value: activeBids },
              { icon: Trophy, label: "Won", value: wonBids },
              { icon: Heart, label: "Favorites", value: favCount },
            ].map((s) => (
          ...
          <div className="space-y-3">
            {bids.length === 0 ? (
              <div className="rounded-2xl glass-strong border border-border/40 p-12 text-center">
                ...
                <h3 className="font-display text-lg font-semibold mb-2">No bids yet</h3>
                <p className="text-muted-foreground text-sm mb-5">Start bidding on live auctions to see your history here.</p>
              ...
              <>
                <div className="rounded-2xl bg-gradient-card border border-border/60 p-4 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">Total amount bid</span>
                  </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {b.won === true && <Badge className="bg-[var(--success)]/20 text-[var(--success)] border-0 text-[9px] px-1.5 py-0 gap-0.5"><Trophy className="h-2 w-2" /> WON</Badge>}
                        {b.won === false && <Badge className="bg-destructive/10 text-destructive border-0 text-[9px] px-1.5 py-0">ENDED</Badge>}
                    ...
                    <div className="text-end shrink-0">
                      ...
                      {b.final_bid && b.final_bid !== b.amount && (
                        <div className="text-xs text-muted-foreground">Final: {formatPrice(b.final_bid)}</div>
                      )}
                    </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                {unreadCount > 0 && (
                  <button className="text-xs text-primary-glow hover:underline" onClick={() => { markAllRead(); }}>Mark all read</button>
                )}
                {notifications.length > 0 && (
                  <button className="text-xs text-muted-foreground hover:underline" onClick={() => { clearNotifications(); }}>Clear all</button>
                )}
              </div>
            </div>
            {notifications.length === 0 ? (
              <div className="rounded-2xl glass-strong border border-border/40 p-12 text-center">
                ...
                <p className="text-muted-foreground text-sm">You'll be notified about bids, auctions, and more.</p>
              </div>
            ) : (
              notifications.map((n) => (
                <div
          ...
          <div>
            {recentCars.length === 0 ? (
              <div className="rounded-2xl glass-strong border border-border/40 p-12 text-center">
                ...
                <h3 className="font-display text-lg font-semibold mb-2">No recently viewed cars</h3>
                <p className="text-muted-foreground text-sm mb-5">Cars you view will appear here.</p>
                <Button asChild className="bg-gradient-primary border-0 text-primary-foreground">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="e.g. BMW 2020 Cairo..."
                  className="ps-10 bg-background/40"
                />
              </div>
              <Button onClick={saveSearch} className="bg-gradient-primary border-0 text-primary-foreground">Save</Button>
            </div>
            {savedSearches.length === 0 ? (
              <div className="rounded-2xl glass-strong border border-border/40 p-12 text-center">
                <Bookmark className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                <h3 className="font-display text-lg font-semibold mb-2">No saved searches</h3>
                <p className="text-muted-foreground text-sm">Save your frequent search terms to quickly access them.</p>
              </div>
            ) : (
          ...
          <div className="rounded-2xl glass-strong border border-border/60 p-6 space-y-5">
            ...
            {editingProfile ? (
              <div className="space-y-4">
                ...
                >
                  <CheckCircle2 className="h-4 w-4 me-2" /> Save Changes
                </Button>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Account Type", value: "Verified Buyer" },
                  { label: "Member Since", value: "January 2025" },
                  { label: "Total Auctions Won", value: wonBids.toString() },
                ].map((f) => (
            ...
            <div className="pt-4 border-t border-border/40 flex flex-wrap gap-3">
              <Button asChild variant="outline" className="glass gap-2">
                <Link to="/favorites"><Heart className="h-4 w-4" /> My Favorites ({favCount})</Link>
              </Button>
              <Button asChild variant="outline" className="glass gap-2">
                <Link to="/sell"><Car className="h-4 w-4" /> List Your Car</Link>
              </Button>
              <Button asChild variant="outline" className="glass gap-2">
                <Link to="/sold"><Trophy className="h-4 w-4" /> Sold Archive</Link>
              </Button>
              <Button
                variant="outline"
